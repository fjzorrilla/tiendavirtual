...
@section('title','PUPPIES HUND || Editar Marcas')
...
<div class="card">
    ...
    <div class="card-body">
      <form method="post" action="{{route('brand.update',$brand->id)}}">
        ...
        <input id="inputTitle" type="text" name="title" placeholder="Ingresar Marcas"  value="{{$brand->title}}" class="form-control">
        @error('title')
        ...
        <div class="form-group">
          ...
          <select name="status" class="form-control">
            ...
            <option value="inactive" {{(($brand->status=='inactive') ? 'selected' : '')}}>Incativo</option>
          </select>
          @error('status')
          <span class="text-danger">{{$message}}</span>
          @enderror
